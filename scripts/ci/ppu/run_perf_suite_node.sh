#!/bin/bash
# Run one node's share of a performance suite, keeping both its log and its
# status.
#
...
# server it lost while rank 0 was measuring.  Each node therefore writes its own
# log into the directory the group shares, which is also what the workflow
# collects as the artifact.
#
# Invoked with bash on purpose: the exit status of the suite has to survive being
# piped into tee, and PIPESTATUS is what makes that exact rather than a
# reconstruction.  Handing back tee's status instead would turn every failed
# suite green.
#
# Usage: run_perf_suite_node.sh <suite> <timeout_per_file> <rank_dir>
...
if [ -z "$suite" ] || [ -z "$timeout_per_file" ] || [ -z "$rank_dir" ]; then
  echo "usage: run_perf_suite_node.sh <suite> <timeout_per_file> <rank_dir>" >&2
  exit 2
fi

node_rank="${SGLANG_PPU_PERF_NODE_RANK:-${NODE_RANK:-0}}"
mkdir -p "$rank_dir"
