...
def _get_k_and_s_torch(
    ...
    nope_idx = (
        nope_off_bytes[:, None]
        + torch.arange(nope_dim, dtype=torch.int64, device=device)[None, :]
    )
    k_nope_bytes = flat_fp8[nope_idx.flatten()].view(n_tokens, nope_dim).contiguous()
    ...
    rope_idx = (
        rope_off_elems[:, None]
        + torch.arange(rope_dim, dtype=torch.int64, device=device)[None, :]
    )
    ...
    s_idx = (
        s_off_bytes[:, None]
        + torch.arange(scale_dim, dtype=torch.int64, device=device)[None, :]
    )
