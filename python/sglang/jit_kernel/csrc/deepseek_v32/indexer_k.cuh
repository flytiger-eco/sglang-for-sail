...
template <typename DType, typename IndexType, typename PosT, bool kUsePDL, int32_t kPageBits>
K_INDEXER_KERNEL void
fused_k_indexer_norm_rope_store_mxfp4(const __grid_constant__ FusedKIndexerNormRopeStoreParams params) {
  using namespace device;
  ...
  constexpr int kLanesPerMxfp4Block = kMxfp4Block / kVecSize;
  constexpr int kPackedBytesPerToken = kHeadDim / 2;
  constexpr int kScaleBytesPerToken = kHeadDim / kMxfp4Block;
  constexpr int64_t kPageBytes = (kPackedBytesPerToken + kScaleBytesPerToken) << kPageBits;
  static_assert(kHeadDim == kWarpThreads * kVecSize);
  static_assert(kRopeDim == kWarpThreads * 2);
  static_assert(kRopeSize <= kWarpThreads);
  ...
  const auto work_id = blockIdx.x * kFusedKIndexerNumWarps + warp_id;
  const bool is_rope_lane = lane_id < kRopeSize;
  const auto block_idx_in_token = lane_id / kLanesPerMxfp4Block;
...
#pragma unroll
  for (int i = 1; i < kVecSize; ++i)
    local_max = math::max(local_max, math::abs(data[i]));
  float amax = warp::reduce_max<kLanesPerMxfp4Block>(local_max);
  ...
  const uint8_t byte0 = e2m1x2_pack_ptx(data[1] * inv_scale, data[0] * inv_scale);
  const uint8_t byte1 = e2m1x2_pack_ptx(data[3] * inv_scale, data[2] * inv_scale);
#else
  const uint8_t byte0 = e2m1_nibble(data[0] * inv_scale) | static_cast<uint8_t>(e2m1_nibble(data[1] * inv_scale) << 4);
  const uint8_t byte1 = e2m1_nibble(data[2] * inv_scale) | static_cast<uint8_t>(e2m1_nibble(data[3] * inv_scale) << 4);
#endif
  const uint16_t packed = static_cast<uint16_t>(byte0) | (static_cast<uint16_t>(byte1) << 8);

  const auto index = static_cast<const IndexType*>(params.indices)[work_id];
  const int32_t page = static_cast<int32_t>(index >> kPageBits);
  const int32_t offset = static_cast<int32_t>(index & ((1 << kPageBits) - 1));
  const auto page_ptr = static_cast<uint8_t*>(params.cache) + page * kPageBytes;
  ...
  if ((lane_id % kLanesPerMxfp4Block) == 0) {
    static_cast<uint8_t*>(scale_ptr)[block_idx_in_token] = ue8m0;
  }
}

template <typename DType, typename IndexType, bool kUsePDL, uint32_t kPageSize>
struct FusedKIndexerNormRopeStoreMXFP4Kernel {
  static constexpr int32_t kPageBits = std::countr_zero(kPageSize);
  static constexpr int64_t kPageBytes = 68ll * kPageSize;
  static_assert(std::has_single_bit(kPageSize), "kPageSize must be a power of 2");

  template <typename PosT>
  static constexpr auto kernel = fused_k_indexer_norm_rope_store_mxfp4<DType, IndexType, PosT, kUsePDL, kPageBits>;
  ...
  static void forward(
    ...
    TensorMatcher({B, kHeadDim}).with_strides({-1, 1}).with_dtype<DType>().with_device(device_).verify(k_input);
    TensorMatcher({-1, -1}).with_strides({kPageBytes, 1}).with_dtype<uint8_t>().with_device(device_).verify(cache);
    TensorMatcher({B}).with_dtype<IndexType>().with_device(device_).verify(indices);
    TensorMatcher({kHeadDim}).with_dtype<float>().with_device(device_).verify(weight);
    TensorMatcher({kHeadDim}).with_dtype<float>().with_device(device_).verify(bias);
    ...
    const auto k_int64 = kernel<int64_t>;
    const auto k = pos_dtype.is_type<int32_t>() ? k_int32 : k_int64;
    LaunchKernel(num_blocks, kFusedKIndexerBlockSize, device_.unwrap()).enable_pdl(kUsePDL)(k, params);
  }
};
