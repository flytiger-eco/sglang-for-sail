...
template <int kBlockN, bool kApplySwigluLimit, int kStages, bool kSwizzle>
...
void silu_and_mul_masked_post_quant_mxfp4_kernel(
  ...
  static_assert(kThreadsPerBlock == kWarpThreads ||
                kThreadsPerBlock == 2 * kWarpThreads ||
                kThreadsPerBlock == 4 * kWarpThreads,
                "expected 1, 2, or 4 warps per block");
...
struct SiluMulMxfp4EP {
  ...
  static void run(
    ...
    LaunchKernel(grid, kBlockN / kElemPerThread, device)(
        silu_and_mul_masked_post_quant_mxfp4_kernel<kBlockN, kApplySwigluLimit, kStages, kSwizzle>,
        params);
  }
};
