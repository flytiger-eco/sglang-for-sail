...
void silu_and_mul_post_quant_mxfp4_kernel(
  ...
  static_assert(kThreadsPerBlock == kWarpThreads / 4 ||
                kThreadsPerBlock == kWarpThreads / 2 ||
                kThreadsPerBlock == kWarpThreads ||
                kThreadsPerBlock == 2 * kWarpThreads ||
                kThreadsPerBlock == 4 * kWarpThreads ||
                kThreadsPerBlock == 8 * kWarpThreads,
                "expected 1/4, 1/2, 1, 2, 4, or 8 warps per block");
  ...
  if (n_offset_elem >= params.N) return;
...
struct SiluMulMxfp4TP {
  ...
  static void run(
    ...
    const int hidden_blocks = N / kBlockN;

    RuntimeCheck(num_tokens > 0, "num_tokens must be positive");
    RuntimeCheck(N % kBlockN == 0,
        "N must be multiple of kBlockN=", kBlockN, ", got N=", N);
    RuntimeCheck(N % 128 == 0, "N must be multiple of 128, got ", N);
    ...
    LaunchKernel(grid, threads, device)(
        silu_and_mul_post_quant_mxfp4_kernel<kBlockN, kApplySwigluLimit>, params);
  }
};
