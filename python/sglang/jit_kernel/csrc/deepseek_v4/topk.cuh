...
SGL_DEVICE void cp_async_commit() { asm volatile("cp.async.commit_group;"); }
SGL_DEVICE void cp_async_wait_all() { asm volatile("cp.async.wait_all;"); }
// Wait until at most kKeep prior cp.async groups remain in flight.
template <uint32_t kKeep>
SGL_DEVICE void cp_async_wait_group() { asm volatile("cp.async.wait_group %0;" :: "n"(kKeep) : "memory"); }
...
#pragma unroll 8
  ...
  __syncthreads();
  {
    issue_chunk(0, 0); cp_async_commit();
    issue_chunk(1, 1); cp_async_commit();
    for (uint32_t ci = 0; ci < num_chunks; ++ci) {
      ...
      __syncthreads();
      issue_chunk(ci + 2, bi); cp_async_commit();
    }
    cp_async_wait_all();
  }
  __syncthreads();
