...
def _ref_mask(topk_ids: torch.Tensor, num_token_non_padded: torch.Tensor) -> torch.Tensor:
    n = topk_ids.shape[0]
...
def test_mask_topk_ids_correctness(num_tokens: int, topk: int, frac_valid: float) -> None:
...
@pytest.mark.parametrize("frac_valid", [0.0, 0.25, 0.5, 1.0])
def test_mask_topk_ids_to_int64_correctness(num_tokens: int, topk: int, frac_valid: float) -> None:
...
def test_mask_topk_ids_to_int64_no_padding() -> None:
    ...
    result = mask_topk_ids_to_int64(topk_ids, torch.tensor([64], dtype=torch.int32, device="cuda"))
...
def test_mask_topk_ids_to_int64_all_padded() -> None:
    ...
    result = mask_topk_ids_to_int64(topk_ids, torch.tensor([0], dtype=torch.int32, device="cuda"))
