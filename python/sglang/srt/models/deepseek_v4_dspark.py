...
class DSparkAttention(MqaAttentionBase):
    ...
    def __init__(
        ...
        # Force unfused wq_a + wkv (DSpark checkpoint is not fused)
        ...
        # v0.5.14 uses tp_rank/tp_size; expose as attn_tp_* for DSpark code.
...
class DSparkV4Stage(DeepseekV4DecoderLayer):
    ...
    def __init__(
        ...
        # DeepseekV4DecoderLayer.__init__ builds a base MQALayer directly (it does
        # not route through _build_self_attn). That base attention runs the full
        # target-model MLA path and, crucially, has a forward signature of
        # (x, positions, ...), while DSparkV4Stage.forward calls
        # self.self_attn(positions, x, ...). The DSpark draft needs its own
        # attention (separate wq_a + wkv, SWA block-KV, per-rank attn_sink) whose
        # forward is (positions, hidden_states, ...). Swap it in here.
        self.self_attn = self._build_self_attn(
            config=config,
            layer_id=layer_id,
            quant_config=quant_config,
            prefix=add_prefix("self_attn", prefix),
            # DSpark draft does not use multi-stream overlap.
            alt_streams=None,
            compress_ratio_override=None,
        )
...
class DeepseekV4ForCausalLMDSpark(nn.Module):
    ...
    def _remap_dspark_weight_name(self, name: str) -> Optional[str]:
        ...
        # PPU uses .weight_scale instead of .weight_scale_inv for non-fp8 quant
        if _is_ppu:
            if self.quant_config and self.quant_config.get_name() == "fp8":
                if (
                    getattr(self.quant_config, "is_fp4_experts", False)
                    and "mlp.experts" in mapped_rest
                ):
