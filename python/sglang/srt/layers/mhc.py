...
def _build_pass_configs():
    ...
    if os.environ.get("TL_MHC_PRE_BIG_FUSE_WITH_NORM_ENABLE_FAST_MATH", "0") in ("1", "true", "True"):
...
def mhc_pre_big_fuse_with_norm_tilelang(
    ...
    hc_mult: int = 4,
    gemm_last_dim: int = -1,
    num_stages_1st: int = _get_env_int("TL_MHC_PRE_BIG_FUSE_WITH_NORM_PIPELINED_NUM_STAGES_1ST", 3),
    num_stages_2nd: int = _get_env_int("TL_MHC_PRE_BIG_FUSE_WITH_NORM_PIPELINED_NUM_STAGES_2ND", 2),
):
    ...
    with T.Kernel(num_tokens, threads=96) as i:
        ...
        if T.get_thread_binding() < 32:
            ...
            for i0_h in T.Pipelined(hidden_size // hidden_block, num_stages=num_stages_1st):
            ...
            for i0_h in T.Pipelined(hidden_size // hidden_block, num_stages=num_stages_2nd):
