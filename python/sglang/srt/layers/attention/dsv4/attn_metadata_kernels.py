...
def expand_prefill_causally(
    *,
    ...
    if req_pool_indices.numel() == 0:
        assert num_tokens == 0, "an empty batch cannot contain real tokens"
        total_tokens = (
            padded_num_tokens
        ...
        return ExpandPrefillCausallyResult(
            seq_lens_casual=torch.ones(total_tokens, **cuda_int32_kwargs),
...
def expand_prefill_causally_triton(
    *,
    ...
    if bs == 0:
        assert num_tokens == 0, "an empty batch cannot contain real tokens"
        # There is no last request to replicate into graph-padding slots. Use
        # the reserved pool row 0 and a harmless one-token causal length.
        return ExpandPrefillCausallyResult(
            seq_lens_casual=torch.ones(total_tokens, dtype=torch.int32, device=device),
            req_pool_indices_repeated=torch.zeros(
                total_tokens, dtype=req_pool_indices.dtype, device=device
            ),
        )
    ...
    _expand_prefill_causally_kernel[(triton.cdiv(total_tokens, BLOCK),)](
        ...
        total_tokens,
        BLOCK=BLOCK,
        BS_P2=triton.next_power_of_2(bs),
    )
    return ExpandPrefillCausallyResult(
        seq_lens_casual=seq_lens_casual,
        req_pool_indices_repeated=req_pool_indices_repeated,
