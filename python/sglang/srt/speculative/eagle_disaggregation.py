...
from sglang.srt.runtime_context import get_server_args, get_spec
...
def build_eagle_disagg_draft_input(
    ...
    if dsa_seed_backend_enabled(get_server_args()):
        ...
        if dsa_indices_list and all(t is not None for t in dsa_indices_list):
            ...
            if should_remap_pd_dsa_seed_to_local_slots():
                ...
                gather_positions = dsa_topk_indices.clamp(
                    min=0, max=table_width - 1
                ).to(torch.int64)
