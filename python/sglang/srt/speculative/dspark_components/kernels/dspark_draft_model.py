...
_STACKED_WEIGHT_CACHE: dict[tuple[torch.nn.Module, ...], _StackedWkvWeight] = {}
...
def _stacked_wkv_weight(*, wkv_linears: list[torch.nn.Module]) -> _StackedWkvWeight:
    # Use the complete ordered module set as the key. Keying only on the first
    # module aliases lists that share their first stage, while storing raw ids
    # would also allow a stale entry to match after Python reuses an object id.
    # Holding the modules themselves makes both the identity and lifetime part
    # of the cache key.
    key = tuple(wkv_linears)
