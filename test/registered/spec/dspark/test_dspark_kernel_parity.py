...
def _case_commit_kv_proj(tc):
    ...
    # Cache identity must include every ordered stage. In particular, two
    # pipelines may share their first linear while using different later
    # stages; reusing the first pipeline's stacked weight would be incorrect.
    cache = dspark_draft_model._STACKED_WEIGHT_CACHE
    cache.clear()
    shared = linears[0]
    first = dspark_draft_model._stacked_wkv_weight(wkv_linears=[shared, linears[1]])
    first_again = dspark_draft_model._stacked_wkv_weight(
        wkv_linears=[shared, linears[1]]
    )
    second = dspark_draft_model._stacked_wkv_weight(wkv_linears=[shared, linears[2]])
    tc.assertIs(first_again, first)
    tc.assertIsNot(second, first)
    tc._eq(second.weight, torch.cat([shared.weight, linears[2].weight], dim=0))
    cache.clear()
...
def _case_expand_prefill_causally(tc):
    ...
    # Empty DP ranks must not launch a zero-batch Triton program. With graph
    # padding they use the reserved request-pool row 0 and causal length 1.
    empty = torch.empty(0, dtype=torch.int32, device=DEVICE)
    for padded_num_tokens in (None, 4):
        got, _ = tc._parity(
            attn_metadata_kernels.ExpandPrefillCausally,
            req_pool_indices=empty,
            seq_lens=empty,
            extend_seq_lens=empty,
            extend_start_loc=empty,
            seq_lens_cpu=None,
            extend_seq_lens_cpu=None,
            num_tokens=0,
            padded_num_tokens=padded_num_tokens,
        )
        expected_size = padded_num_tokens or 0
        tc.assertEqual(got.seq_lens_casual.numel(), expected_size)
        tc.assertTrue(bool((got.seq_lens_casual == 1).all()))
        tc.assertTrue(bool((got.req_pool_indices_repeated == 0).all()))
