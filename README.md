...
SGLang-for-SAIL is a PPU-adapted inference engine based on SGLang v0.5.15.post1.
...
Before installing SGLang-for-SAIL v0.5.15.post1, make sure the SAIL SDK and required runtime components are available in your environment.
...
sudo docker run -it \
  --name SGLang-for-SAIL_0.5.15.post1 \
  --privileged \
  --ipc=host \
...
```bash
pip install sglang==0.5.15.post1 --force-reinstall --no-deps -i https://pkg.flytiger-eco.com/artifactory/api/pypi/pypi_index/simple
```
...
The command should print `0.5.15.post1`.
